                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-success btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#exampleModal" onclick="openEditModal({{ json_encode($linha) }})">
                                    <i class="fa fa-pencil"></i>

                                    <form action="{{ route('jogo_excluir', ['jogo' => $linha->id]) }}" method="POST"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('jogo_atualizar', ['jogo' => $linha->id]) }}"
                        enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" id="user-id" value="{{ $linha->id }}">
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" name="nome" id="user-name"
                                value="{{ $linha->nome }}" required>
                            <label for="user-name">Nome</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control" name="descricao" id="user-descricao"
                                value="{{ $linha->descricao }}" required>
                            <label for="user-descricao">Descrição</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="number" class="form-control" name="preco" id="user-preco"
                                value="{{ $linha->preco }}" required>
                        ...
                        <div class="form-floating mb-3">
                            <select class="form-control" name="jogo_img" id="jogo-img-perfil" required>
                                ...
                                @foreach ($imagens as $img)
                                    <option value="{{ $img->id }}"
                                        data-img-src="{{ asset('storage/' . $img->img) }}">
                                        {{ $img->img_nome }}
                                    </option>
                                @endforeach
                            </select>
